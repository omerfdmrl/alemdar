#ifndef ALEMDAR_LAYER_MEAN_POOLING_H

#define ALEMDAR_LAYER_MEAN_POOLING_H

ALEMDAR_DEF void layer_mean_pooling_forward(Layer layer);
ALEMDAR_DEF Layer layer_mean_pooling(size_t inputSize, int poolSize);

void layer_mean_pooling_forward(Layer layer) {
    int index = 0;
    float mean_val;
...
Layer layer_mean_pooling(size_t inputSize, int poolSize) {
    ...
    l.forward = layer_mean_pooling_forward;
    l.bias = vec_alloc(0);
...
#endif // !ALEMDAR_LAYER_MEAN_POOLING_H