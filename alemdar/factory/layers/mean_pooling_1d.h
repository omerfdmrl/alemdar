#ifndef ALEMDAR_LAYER_MEAN_POOLING_1D_H

#define ALEMDAR_LAYER_MEAN_POOLING_1D_H

ALEMDAR_DEF void layer_mean_pooling_1d_forward(Layer layer);
ALEMDAR_DEF Layer layer_mean_pooling_1d(size_t inputSize, int poolSize);

void layer_mean_pooling_1d_forward(Layer layer) {
    int index = 0;
    float mean_val;
...
Layer layer_mean_pooling_1d(size_t inputSize, int poolSize) {
    ALEMDAR_ASSERT(poolSize > 0);
    Layer l;
    l.name = MeanPooling1D;
    ...
    l.forward = layer_mean_pooling_1d_forward;
    l.bias = vec_alloc(0);
...
#endif // !ALEMDAR_LAYER_MEAN_POOLING_1D_H